...
**`apps/web/src/app/labs/[id]/edit/elements/` (M2.2 + M2.2 polish):**
- `text-element.tsx` — TipTap-based rich text element (StarterKit + Color + TextStyle + TextAlign + Tables + Placeholder)
- `equation-element.tsx` — KaTeX view + popup MathLive editor with Word-style font controls
...
*Last updated: 2026-05-07. M2.2 + M2.2-polish complete (Day 3 weekly recap covers the polish details). Starting M2.3 (Quiz blocks) next session, with one queued micro-feature: inline equations inside text boxes (custom TipTap node + reuse existing equation popup).*
